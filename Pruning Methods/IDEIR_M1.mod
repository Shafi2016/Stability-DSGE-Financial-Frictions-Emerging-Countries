%% IDEIR Model with multiplicative Risk Premium, Call it IDEIR(m)
...
parameters  gamma, omega, rho, sigmae, delta, psi, alpha, phi, beta, r_w, d_bar;
		 ...
		 omega  = 1.6;
          delta  = 0.03;
          //psi  = 0.0019;// fernandez-villaverde_et_al
          // psi  = 0.0019;// andreasen
          // psi  = 0.0019;// den_haan_de_wind, ORDER 3
         // psi  = 0.0019;//lan_meyer-gohde
         //psi  = 0.0019;//without pruning
           psi  = 0.0019; //NLMA
            sigmae = 0.0201;
         beta   = 1/(1+r_w);
...
model;
    ...
    ca = (1/exp(y))*(d-d(-1));                                   
         %Risk premium
     riskpremium = exp(psi*(d-d_bar))+ exp(psi*(d-d_bar))*d*psi; 
