%% EDEIR Model with multiplicative Risk Premium, Call it EDEIR(m)
...
parameters  gamma, omega, rho, sigmae, delta, psi, alpha, phi, beta, r_w, d_bar;
		 ...
		 omega  = 1.6;
          delta  = 0.03;
          // psi  = 0.00263;  //lan_meyer-gohde
          // psi  = 0.00263;  //adreasen
          // psi  = 0.00263;  //fernandez-villaverde_et_al
          //psi  = 0.00263;  //den_haan_de_wind,
          //psi    = 0.00263;//without pruning
            psi  = 0.00263;//NMLA
         sigmae = 0.0201;
         beta   = 1/(1+r_w);
...
model;
    ...
    ca = (1/exp(y))*(d-d(-1));                                   
       % Risk premium    
      riskpremium = exp(psi*(d-d_bar));    // EDEIR Multiplicative
       %country interest rate
      exp(r) = r_w*riskpremium;   // multiplicative
